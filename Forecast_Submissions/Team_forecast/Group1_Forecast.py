...
url_1 = "https://waterdata.usgs.gov/nwis/dv?cb_00060=on&format=rdb&site_no=0950\
        6000&referred_module=sw&period=&begin_date=1990-01-01&end_date=\
        2021-11-13"
...
# %%
# Map!
file = os.path.join('/Users/sierra/Desktop/Desktop - Sierra’s MacBook Pro\
        /Fall 2021/HASTools/homework-sbettis319/data/Average_Annual_\
        Precipitation_-_AZ_(1961_-_1990)-shp/Average_Annual_Precipitation\
        _-_AZ_(1961_-_1990).shp')
...
file = os.path.join('/Users/sierra/Desktop/Desktop - Sierra’s MacBook Pro\
        /Fall 2021/HASTools/homework-sbettis319/data/Shape')
fiona.listlayers(file)
...
data = ('/Users/sierra/Desktop/Desktop - Sierra’s MacBook Pro/Fall 2021/\
        HASTools/homework-sbettis319/data/X107.2.20.129.315.18.5.30.nc')
...
# Grab the data
start_date = '20211112'
base_url = "https://www.ncei.noaa.gov/data/global-forecast-system\
        /access/grid-003-1.0-degree/forecast/202111/"+start_date+"/"

precip = np.zeros(113)

for i in range(48, 343, 3):
    if i < 10:
    ...
    req.urlretrieve(base_url+filename, 'temp.grb2')
    dataset = pg.open('temp.grb2')
    if str(dataset[596])[0:32] == '596:Total Precipitation:kg m**-2':
        precip[int(i/3-1)] = dataset[596].values[34, 248]

date_time = pd.date_range(start='2021-11-14 0:00', periods=113, freq='3H')
...
def forecasts(month1, month2, day_start, day_end, precip_chance):
    '''
        This function determines the week 1 and week 2 forecast predictions
        based on the forecasted precip in Camp Verde
        ...
        "precip_chance" represents the total forecasted amount of precipitation
        in inches (int)

        Outputs:
        This function returns a print statement that provides the forecasted
        flows for week 1 and week 2.
        '''
    # If there is no forecasted precip, use this chunk (lines 119 to 127):
    # week_flow = data1_i['flow'][('2021-' + str(month1) + '-' +\
    # str(day_start)):('2021-' + str(month2) + '-' + str(day_end))]
    ...
    week_flow1 = data1[(data1['year'] == 2021) & (data1['month'] == 11)
                       & (data1['day'] >= day_start)
                       & (data1['day'] <= day_end)]
...
forecasts(11, 11, 7, 13, precip_fore)  # precip_fore is the amount of
# precipitation that is forecasted in inches
