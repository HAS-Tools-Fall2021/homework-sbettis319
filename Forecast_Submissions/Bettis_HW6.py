...
ax.plot(data['datetime'], data['flow'], color='blue',
        linestyle='solid', label='daily flow')
...
# %% graph 1
#Timeseries of observed weekly flow values
...
month_data = data[data['month'] == m]
plot_title = 'Month ' + str(m)
ax[0].hist(np.log10(month_data['flow']), bins=20, edgecolor='yellow', color='pink')
...
for m in range(9,13):
        ...
        plot_title = 'Month ' + str(m)
        ax[axi].hist(np.log10(month_data['flow']), bins=30,
           edgecolor='grey', color='magenta')
        ax[axi].set(xlabel='Log Flow cfs', ylabel='count', title=plot_title)
        axi=axi+1
plt.show()
# %% graph 4
# Plot the october flows for the last year
fig, ax = plt.subplots()
...
fig, ax = plt.subplots()
ax.plot(data['datetime'], data['flow'], label='flow', color = 'purple')
ax.set(title="Observed Flow", xlabel="Date", ylabel="Weekly Avg Flow [cfs]",
        yscale='log', xlim=[datetime.date(2015, 10, 1), datetime.date(2020, 10, 14)])
ax.legend()
plt.show()
# %%
# graph 6
#Boxplot of flows by month 
fig, ax = plt.subplots()
ax = sns.boxplot(x="month", y="flow",  data=data,
                 linewidth=0.5)
ax.set(yscale='log')
